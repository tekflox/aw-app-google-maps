...
Unlike aw-app-google-workspace-mcp, the entry is written whether or not a key is
configured. The difference is that these tools degrade *usefully* without one:
they answer with a one-line "no API key configured, set it in Settings", which
is a better failure than the tools not existing at all. An OAuth server can't do
that — it fails deep inside a token exchange with an error about Google.
"""
...
def write_mcp_json(package_dir: str, port: int | None = None) -> dict:
    ...
    triggered it. An unconditional rewrite on every activate is a reload loop.
    """
